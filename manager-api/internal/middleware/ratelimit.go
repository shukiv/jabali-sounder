...
// Defaults for login throttling. The control plane has a single admin account,
// so one target = full compromise; failed logins must be throttled (OWASP
...
// locked out for lockout and further attempts get 429 with Retry-After.
...
// NewLoginLimiter builds a limiter. A nil now defaults to time.Now; non-positive
// tuning values fall back to the package defaults.
func NewLoginLimiter(maxFailures int, lockout, window time.Duration, now func() time.Time) *LoginLimiter {
	if maxFailures <= 0 {
...
// DefaultLoginLimiter is the production-tuned limiter.
func DefaultLoginLimiter() *LoginLimiter {
	return NewLoginLimiter(defaultLoginMaxFailures, defaultLoginLockout, defaultLoginWindow, nil)
}
