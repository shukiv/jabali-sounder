import { Card, Table, Typography, Tag, Button, Empty, Statistic, Row, Col } from "antd";
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col xs={12} sm={8}>
          <Card><Statistic title="Servers at risk" value={data?.servers_at_risk ?? 0} suffix={`/ ${data?.servers_total ?? 0}`} valueStyle={{ color: (data?.servers_at_risk ?? 0) ? "#d48806" : "#3f8600" }} /></Card>
        </Col>
        <Col xs={12} sm={8}>
          <Card><Statistic title="Critical violations" value={critical} valueStyle={{ color: critical ? "#cf1322" : "#3f8600" }} /></Card>
        </Col>
        <Col xs={24} sm={8}>
          <Card><Statistic title="Total violations" value={data?.total ?? 0} /></Card>
        </Col>
      </Row>
      <Card>
